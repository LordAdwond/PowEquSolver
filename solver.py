...
class solver:
    ...
    @staticmethod
    def solve_quadratic(b: float, c: float) -> list:  # solving quadratic equation x^2+bx+c=0
        D = b * b - 4 * c
        ans = []
        if D >= 0:
            ans = [(-b - np.sqrt(D)) / 2, (-b + np.sqrt(D)) / 2]
        else:
            ans = [complex(-0.5 * b, -0.5 * np.sqrt(-D)), complex(-0.5 * b, 0.5 * np.sqrt(-D))]
    ...
    def solve_cubic(b: float, c: float, d: float, eps=0.001) -> list:  # solving cubic equation x^3+bx^2+cx+d=0
        ...
        return solver.solve_linear(1, x0[0]) + solver.solve_quadratic(x0[1], x0[2])

    @staticmethod
    def solve_forth(b:float, c:float, d:float, e:float, eps=0.001) -> list:
        def f(x):
            b1 = x[0]
            c1 = x[1]
            b2 = x[2]
